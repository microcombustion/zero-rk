...
//  expWorkArray = new double[max(nFromKeqStep,nDistinctArrhenius)];
  ...
  allocSize = ((allocSize + 3)/4)*4; //round to next even multiple of 4
  expWorkArray = (double*)_aligned_malloc(sizeof(double)*allocSize,16);
  ...
  if(nArrheniusStep > 0) {
    ...
    for(j=0; j<nStep; j++) {
	  ...
	  paramListTmp[k].A = ckrobj->reactions[rxnIdx].kf.A*
	    pow(convertC,(1.0-(double)netobj->getOrderOfStep(j)));
  ...
  for(j=0; j<nRxn; j++) {
    ...
    if(ckrobj.reactions[j].isFalloffRxn) {
      ...
      if(strcmp(ckrobj.reactions[j].thirdBody.c_str(),"M")!=0) {
        ...
        printf("INFO: reaction %d has a fall-off third body (%s) != (M)\n",
               j,ckrobj.reactions[j].thirdBody.c_str());
        printf("      with species id = %d.\n",
               falloffRxnList[k].falloffSpcIdx);
      }
	...
	falloffRxnList[k].etbSpcEff.resize(nEnh);
      ...
      if(nEnh > 0 &&
        ...
        printf("WARNING: reaction %d has a single falloff species (+%s)\n",
               j,ckrobj.reactions[j].thirdBody.c_str());
      ...
      log_e_Alow = ckrobj.reactions[j].kf_aux.A*
        pow(convertC,
	   -(double)netobj.getOrderOfStep(falloffRxnList[k].fwdStepIdx));
      ...
      if(ckrobj.reactions[j].falloffType == ckr::Lindemann) {
        ...
        falloffRxnList[k].falloffType = 1000;
	...
	// T*
        ...
        // Warning statments for possibly confusing Troe forms
        if(ckrobj.verbose)
        {
            isNonStandardTroe(k,  // falloff index
                              j); // reaction index
        }

      } // end of if reaction j is a 3 or 4 parameter Troe reaction
      else {
        printf("ERROR: rxn %d falloff type %d not recognized\n",
...
void rate_const::updateFromKeqStep()
{
  ...
  for(j=0; j<nFromKeqStep; j++)
	...
	{reacSum+=Gibbs_RT[fromKeqStepList[j].reacSpcIdx[k]];}

      prodSum=0.0;
      for(k=0; k<fromKeqStepList[j].nProd; ++k)
	{prodSum+=Gibbs_RT[fromKeqStepList[j].prodSpcIdx[k]];}
      
      expWorkArray[j]=-(reacSum-prodSum+fromKeqStepList[j].nDelta*log_e_PatmInvRuT);
    }
  fast_vec_exp(expWorkArray,nFromKeqStep+nFromKeqStep%2);
  for(j=0; j<nFromKeqStep; j++)
    {
      Kwork[fromKeqStepList[j].stepIdx]=expWorkArray[j]*Kwork[fromKeqStepList[j].fwdStepIdx];
    }
}
...
void rate_const::updateFalloffRxn(const double C[])
{
  ...
  for(j=0; j<nFalloffRxn; j++) {
    ...
    if(falloffRxnList[j].falloffType==2000 ||
       falloffRxnList[j].falloffType==3000) {
      ...
      if(falloffRxnList[j].falloffType==3000) {
        // 4-parameter Troe
      ...
      // isNonStandardTroe(...) should halt the program if it is encountered.

      //if(falloffRxnList[j].falloffType==3000 &&
      //   falloffRxnList[j].param[3] == 0.0 && 
      //   falloffRxnList[j].param[5]  < 0.0) {
      //
      //  Fcenter = exp(-T/T***) -(T*)*exp(-T**/T)
      //  Fcenter=exp(-Tcurrent/falloffRxnList[j].param[4])
      //            -falloffRxnList[j].param[5]
      //            *exp(-falloffRxnList[j].param[6]*invTcurrent);
      //  if(Fcenter < 1.0e-300) {
      //    Fcenter=1.0e-300;
      //  }
      //  fTerm=log10(Fcenter);
...
//
int rate_const::isNonStandardTroe(const int falloffId,
                                  const int rxnId) const
{
  int nonStdFlag=0;

  if(falloffRxnList[falloffId].param[3] <= 0.0 || 
     falloffRxnList[falloffId].param[3] >= 1.0) {
    nonStdFlag += 1;
  }
  if(falloffRxnList[falloffId].param[4] <= 0.0) {
    nonStdFlag += 2;
  }
  if(falloffRxnList[falloffId].param[5] <= 0.0) {
    nonStdFlag += 4;
  }
  if(falloffRxnList[falloffId].falloffType==3000) {
    if(falloffRxnList[falloffId].param[6] <= 0.0) {
      nonStdFlag += 8;
    }
  }
  if(nonStdFlag > 0) {
    printf("INFO: Reaction %d may not have the intended centering function.\n",
           rxnId+1);
    printf("      Troe centering fucntion Fc(T) being used:\n");
    printf("        Fc(T) = (%.6g)exp(-T/(%.6g))\n",
           1.0-falloffRxnList[falloffId].param[3],
           falloffRxnList[falloffId].param[4]);
    printf("                +(%.6g)exp(-T/(%.6g))\n",
           falloffRxnList[falloffId].param[3],
           falloffRxnList[falloffId].param[5]);
    if(falloffRxnList[falloffId].falloffType==3000) {
      printf("                +exp(%.6g/T)\n",
             -falloffRxnList[falloffId].param[6]);
    }  
    printf("\n");
    printf("      In some older mechanisms, non-positive parameters were\n"
                                  ...
                                  info_net &netobj)
{
  ...
  for(int j=0; j<nRxn; ++j) {
    if(ckrobj.reactions[j].kf.type == ckr::PLogInterpolation) {
      ...
      log_e_A = ckrobj.reactions[j].kf.A_plog;
      ...
      for(int k=0; k<num_pts; ++k) {
        ...
        log_e_A[k]*=pow(convertC,
                        (1.0-(double)netobj.getOrderOfStep(step_id)));
        // there is no 3rd body units correction since PLOG and 3rd body
        // reactions are treated as being mutually exclusive
        log_e_A[k] = log(log_e_A[k]);
      }
      PLogReaction current_reaction(PLogReaction(step_id,
                                                 num_pts,
                                                 pres.data(),
                                                 log_e_A.data(),
                                                 Tpow.data(),
                                             ...
                                             const double log_e_pressure)
{
  for(int j=0; j<nPLogInterpolationStep; ++j) {
   expWorkArray[j] = 
     plogInterpolationStepList[j].GetRateConstantLogE(invTcurrent,
                                                      log_e_Tcurrent,
                                                      pressure,
                                                      log_e_pressure);
  }
  fast_vec_exp(expWorkArray,
               nPLogInterpolationStep+nPLogInterpolationStep%2);
  for(int j=0; j<nPLogInterpolationStep; ++j) {
